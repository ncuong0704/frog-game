function CGame(oData) {
  ...
  this._init = function () {
    ...
    _oHammer.get("swipe").set({ threshold: 10 });
    ...
    //if(s_bMobile){
    //    s_oCanvas.addEventListener("touchend", function(e){s_oGame.onUpPress(e);});
    // } else{
    s_oStage.on("click", function (e) {
      s_oGame.onUpPress(e);
    });
    // }
  };
  ...
  this.onUpPress = function (e) {
    // console.log(e);
    // console.log("onUpPress", 1995);
    if (e && e.stageY < 90) {
      return;
    }
    if (_bBlockMovement) {
  ...
  this.updateLogicPos = function () {
    console.log("updateLogicPos", 1995);

    for (var i = 0; i < _iNumVertCel; i++) {
      for (var j = 0; j < _iNumHorizCel; j++) {
        if (
          _oFrog.getPos().x >= _aGridMatrix[i][j].x - _oPosOffset.x / 2 &&
          _oFrog.getPos().x <= _aGridMatrix[i][j].x + _oPosOffset.x / 2
        ) {
          _oCurLogicPos.col = j;
        }
        ...
        if (
          _oFrog.getPos().y >= _aGridMatrix[i][j].y - _oPosOffset.y / 2 &&
          _oFrog.getPos().y <= _aGridMatrix[i][j].y + _oPosOffset.y / 2
        ) {
          _oCurLogicPos.row = i;
        }
      ...
      if(!win){
        ...
        setTimeout(()=>{
          ...
          const playContinue = ()=>{
            this._continueGame()
            closePopupVoucher()
            btnPlayContinue.removeEventListener("click", playContinue);
          }
          btnPlayContinue.addEventListener("click", playContinue)
        }, 500)
        win = true;
      }
    }
  ...
  this.unload = function () {
    ...
    if (s_bMobile) {
      s_oCanvas.removeEventListener("touchend", s_oGame.onUpPress, false);
    } else {
      s_oStage.off("pressup", function () {
        s_oGame.onUpPress();
      });
    }
  ...
  this._pauseGame =  function(){
    currentLife = _iLives
    _iLives = 0;
    _bLevelInProgress = false;
    createjs.Tween.removeAllTweens();
  }
  this._continueGame = function(){
    _iLives = currentLife;
    _bLevelInProgress = true;
    ...
    if (_iLives === 0) {
      ...
      setTimeout(() => {
        const clickConfirm = ()=>{
            ...
            var count = 0;
            allInput.forEach(function(cur){
              console.log($(cur).attr("data-type"));
              if($(cur).attr("data-type") == "required"){
                if(!cur.value){
                    count++
                    $(cur).parents(".form-add-life__input").find(".form-add-life__error").text("Vui lòng không bỏ trống!")
                }else{
                  $(cur).parents(".form-add-life__input").find(".form-add-life__error").text("")
                }
              }else if($(cur).attr("data-type") == "email"){
                if(!cur.value){
                  count++
                  $(cur).parents(".form-add-life__input").find(".form-add-life__error").text("Vui lòng không bỏ trống!")
                }else{
                  var mailformat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
                  if(!cur.value.match(mailformat)){
                    count++
                    $(cur).parents(".form-add-life__input").find(".form-add-life__error").text("Vui lòng điền đúng email!")
                  }else{
                    $(cur).parents(".form-add-life__input").find(".form-add-life__error").text("")
                  }
                }
            ...
            if(count == 0){
                ...
                confirmBtn.removeEventListener("click", clickConfirm);
            }
  ...
  this._checkNewLevel = function () {
    console.log("_checkNewLevel", 1995);
  ...
  this.addElemToRemoveList = function (oElem) {
    // console.log("addElemToRemoveList", 1995);
  ...
  this.freeCell = function (iCell) {
    console.log("freeCell", 1995);

    _aNest[iCell] = "free";
  };

  this._generateFly = function () {
    // console.log("_generateFly", 1995);
  ...
  this._generateNewFrog = function () {
    // console.log("_generateNewFrog", 1995);
    ...
    _oCurLogicPos = { row: FROG_STARTING_LOGIC_POS.row, col: FROG_STARTING_LOGIC_POS.col };
    _oFrog = new CFrog(
      _aGridMatrix[_oCurLogicPos.row][_oCurLogicPos.col].x,
      _aGridMatrix[_oCurLogicPos.row][_oCurLogicPos.col].y,
      _oFrogContainer
    );
    _oInterface.refreshBar();
    _bStartGame = true;
  ...
  this._generateSupport = function () {
    // console.log("_generateSupport", 1995);
  ...
  this._checkFrogInWater = function () {
    console.log("_checkFrogInWater", 1995);
    ...
    if (_bStartGame) {
      ...
      if (_oCurLogicPos.row === 14) {
        if (_aNest[_oCurLogicPos.col] === "free" || _aNest[_oCurLogicPos.col] === "fly") {
          playSound("frog_arrived", 1, 0);

          var bFlyScore = false;
          if (_aNest[_oCurLogicPos.col] === "fly") {
            playSound("eat_fly", 1, 0);
  ...
  SCORE_DEATH = 0; //oData.score_death;
